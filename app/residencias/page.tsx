...
export default function ResidenciasPage() {
  ...
  const inputStyle: React.CSSProperties = { width: "100%", background: lightTheme ? "rgba(0,0,0,0.03)" : "rgba(var(--theme-contrast-rgb),0.04)", border: `1px solid ${C.borderDim}`, color: C.textPrimary, fontFamily: "inherit", fontSize: "12px", padding: "0.52rem 0.6rem", outline: "none", boxSizing: "border-box", letterSpacing: "0.02em" };
  const btnStyle: React.CSSProperties = { background: "transparent", border: `1px solid ${C.border}`, color: C.gold, fontSize: "10px", letterSpacing: "0.18em", padding: "0.52rem 0.65rem", cursor: "pointer", textTransform: "uppercase", fontFamily: "inherit", fontWeight: 700 };
  ...
  return <>
    <div className="mob-page-desktop" style={{ minHeight: "100vh", background: C.pageBg, color: C.textPrimary, fontFamily: "'Montserrat','Helvetica Neue',sans-serif", opacity: mounted ? 1 : 0, transition: "opacity 0.6s ease" }}>
      ...
      <main style={{ padding: "2.75rem 3.25rem", maxWidth: "1560px", margin: "0 auto" }}>
        ...
        <div style={{ marginTop: "1rem", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.95rem" }}><Metric label="Residências ativas" value={String(rows.filter(r => r.ativo === 1).length)} C={C} /><Metric label="Clientes" value={String(new Set(rows.filter(r => r.ativo === 1).map(r => r.cliente_nome).filter(Boolean)).size)} C={C} /><Metric label="Custo residência médio" value={euro(avg(rows.filter(r => r.ativo === 1).map(r => r.custo_interno).filter(Boolean)))} C={C} /><Metric label="Faturação residência média" value={euro(avg(rows.filter(r => r.ativo === 1).map(r => r.valor_cliente).filter(Boolean)))} C={C} /></div>
      </main>
    </div>
    ...
    <div className="mob-shell" style={{ fontFamily: "'Montserrat','Helvetica Neue',sans-serif", color: C.textPrimary, opacity: mounted ? 1 : 0, transition: "opacity 0.6s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.9rem 1.1rem", borderBottom: `1px solid ${C.borderDim}`, background: C.pageBg, position: "sticky", top: 0, zIndex: 10 }}><span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.2rem", letterSpacing: "0.35em", color: C.gold, fontWeight: 300 }}>LLE</span><ThemeSwitcher lightTheme={lightTheme} setLightTheme={setLightTheme} style={{ fontSize: "12px", padding: "0.4rem 0.5rem" }} /></div>
      <div style={{ padding: "1.2rem", borderBottom: `1px solid ${C.borderDim}` }}><p style={{ fontSize: "11px", letterSpacing: "0.35em", color: C.textSec, textTransform: "uppercase", fontWeight: 700 }}>Residências Ativas</p><p style={{ fontSize: "13px", color: C.textMuted, marginTop: "0.4rem" }}>Regras recorrentes por cliente/local.</p></div>
      <div className="mob-list">{rows.map(row => <div key={row.id} style={{ padding: "1.2rem", borderBottom: `1px solid ${C.borderDim}`, opacity: row.ativo === 0 ? 0.45 : 1 }}><div style={{ fontSize: "15px", fontWeight: 700, color: C.textPrimary }}>{row.nome}</div><div style={{ fontSize: "13px", color: C.textSec, marginTop: "0.35rem" }}>{row.cliente_nome || "Sem cliente"} · {row.local || "Sem local"} · {row.servico}</div><div style={{ fontSize: "13px", color: C.textSec, marginTop: "0.35rem" }}>Residência: custo {euro(row.custo_interno)} · faturação {euro(row.valor_cliente)}<br />Evento de Residência: custo {euro(row.custo_evento)} · faturação {euro(row.valor_evento)}</div>{row.performer_padrao_nome && <div style={{ fontSize: "12px", color: C.textMuted, marginTop: "0.35rem" }}>Performer: {row.performer_padrao_nome}</div>}</div>)}</div>
      <MobTabBar active="residencias" role="admin" lightTheme={lightTheme} />
    </div>
    <div style={{ position: "fixed", bottom: "2rem", right: "2rem", background: C.surface, border: `1px solid ${C.border}`, color: C.gold, fontSize: "12px", letterSpacing: "0.25em", padding: "1rem 1.5rem", zIndex: 2000, transform: toast ? "translateX(0)" : "translateX(200%)", transition: "transform 0.3s ease", textTransform: "uppercase", fontWeight: 600 }}>{toast}</div>
  </>;
}

function avg(values: number[]) { return values.length ? values.reduce((s, v) => s + v, 0) / values.length : 0; }
function Metric({ label, value, C }: { label: string; value: string; C: typeof C_Dark }) { return <div style={{ background: C.surface, border: `1px solid ${C.borderDim}`, padding: "1.2rem" }}><div style={{ fontSize: "9px", letterSpacing: "0.35em", color: C.textMuted, textTransform: "uppercase", marginBottom: "0.5rem" }}>{label}</div><div style={{ fontSize: "18px", fontWeight: 700, color: C.textPrimary }}>{value}</div></div>; }
