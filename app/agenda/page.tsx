...
export default function AgendaPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [r, ar, cr, lr] = await Promise.all([
        getAllAgenda().catch(() => ({ success: false, data: [] })),
        getAllArtistasAgenda().catch(() => ({ success: false, data: {} })),
  ...
  const handleCancel = async (e: AgendaEvent) => {
    try {
      ...
      if (!res.success) { showToast("Erro ao cancelar evento"); return; }
      await load();
  ...
  const handleCancelFromModal = async () => {
    if (modal.editing) {
      try {
        ...
        if (!res.success) { showToast("Erro ao cancelar"); return; }
        closeModal();
        await load();
        showToast("Evento cancelado", {
          label: "Undo",
    ...
    <div className="mob-shell" style={{ fontFamily: "'Montserrat','Helvetica Neue',sans-serif", color: "#F5F0E8", opacity: mounted ? 1 : 0, transition: "opacity 0.6s ease" }}>
      ...
      {mobFilterOpen && (
        <div style={{ background: "#131108", borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "0.75rem 1rem", flexShrink: 0 }}>
          {!mobFilterCategory ? (
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                <span style={{ fontSize: "8px", letterSpacing: "0.3em", color: C.goldDim, textTransform: "uppercase" }}>Equipa</span>
                <button onClick={() => setMobFilterCategory("")} style={{ background: "transparent", border: "none", color: "rgba(245,240,232,0.3)", cursor: "pointer", fontSize: "12px" }}>✕</button>
              </div>
          ...
          ) : mobFilterCategory === "artista" ? (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                <span style={{ fontSize: "8px", letterSpacing: "0.3em", color: C.goldDim, textTransform: "uppercase" }}>Artistas</span>
                <button onClick={() => setMobFilterCategory("")} style={{ background: "transparent", border: "none", color: "rgba(245,240,232,0.3)", cursor: "pointer", fontSize: "12px" }}>✕</button>
              </div>
              <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                <span style={{ fontSize: "8px", letterSpacing: "0.3em", color: C.goldDim, textTransform: "uppercase" }}>Clientes</span>
                <button onClick={() => setMobFilterCategory("")} style={{ background: "transparent", border: "none", color: "rgba(245,240,232,0.3)", cursor: "pointer", fontSize: "12px" }}>✕</button>
              </div>
              <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
      ...
      <div className="mob-list">
        ...
        {allRows.map((row) => {
          ...
          const bsColor = bsColors[e.billing_status || "Contacto"] || "rgba(245,240,232,0.3)";
          return (
            <div key={e.id} className={`mob-card${e.cancelled ? " is-folga" : ""}`} style={{ opacity: e.cancelled ? 0.45 : 1, borderLeft: e.event_date === todayStr ? `3px solid ${C.gold}` : undefined }}>
              <div className="mob-date-bubble" onClick={() => userRole !== "limited_novalues" && openEdit(e)} style={{ cursor: userRole !== "limited_novalues" ? "pointer" : "default" }}>
                <div className="mob-date-day" style={e.event_date === todayStr ? { color: C.gold, fontWeight: 700 } : undefined}>{ed.getDate()}</div>
                <div className="mob-date-weekday">{ed.toLocaleDateString("pt-PT", { weekday: "short" })}</div>
              </div>
              <div className="mob-card-body" onClick={() => userRole !== "limited_novalues" && openEdit(e)} style={{ cursor: userRole !== "limited_novalues" ? "pointer" : "default" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "2px" }}>
                  {icons && <span style={{ fontSize: "13px", letterSpacing: "1px" }}>{icons}</span>}
                  <div className={`mob-card-title${e.cancelled ? " cancelled" : ""}`}>{e.title.replace(/^\p{Emoji}[\p{Emoji}\u200d\s]*/u, "")}</div>
                </div>
                {e.venue && <div className="mob-card-meta" style={{ color: "rgba(201,169,110,0.7)" }}>📍 {e.venue}</div>}
                {e.time_range && <div className="mob-card-meta">🕐 {e.time_range}</div>}
                <div className="mob-card-badges" style={{ marginTop: "4px" }}>
                  <span className="mob-badge" style={{ background: `${bsColor}18`, color: bsColor }}>
                    <span className="mob-badge-dot" style={{ background: bsColor }} />
                    {e.cancelled ? "Cancelado" : (e.billing_status || "Contacto")}
                  </span>
                </div>
              </div>
              <div className="mob-card-right" style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px" }}>
        ...
        <div style={{ ...modalStyle, width: "400px", maxWidth: "95vw" }}>
          ...
          {waPeriodMode === "custom" && (
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem", alignItems: "center" }}>
              <input type="date" value={waCustomStart} onChange={e => setWaCustomStart(e.target.value)} style={{ ...inputStyle, flex: 1 }} />
              <span style={{ color: C.textMuted, fontSize: "10px" }}>até</span>
              <input type="date" value={waCustomEnd} onChange={e => setWaCustomEnd(e.target.value)} style={{ ...inputStyle, flex: 1 }} />
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: "0.75rem", justifyContent: "flex-end" }}>
            ...
            <button onClick={handleCopyAgenda} style={{ background: "#5DCAA5", border: "none", color: "#0C0B09", fontSize: "9px", letterSpacing: "0.3em", fontWeight: 700, padding: "0.6rem 1.5rem", cursor: "pointer", fontFamily: "inherit", textTransform: "uppercase" }}>Copiar</button>
          </div>
        </div>
      </div>
    )}
      ...
      <div onClick={e => e.target === e.currentTarget && setWaModal(false)} style={overlayStyle}>
        <div style={{ ...modalStyle, width: "520px", maxWidth: "95vw", maxHeight: "85vh", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
            <p style={{ fontSize: "8px", letterSpacing: "0.4em", color: "rgba(201,169,110,0.6)", textTransform: "uppercase", fontWeight: 600 }}>Agenda — WhatsApp</p>
          ...
          {waCopied && <div style={{ marginBottom: "0.75rem", fontSize: "9px", letterSpacing: "0.25em", color: "#5DCAA5", textTransform: "uppercase" }}>✓ Copiado. Pode colar no WhatsApp.</div>}
          ...
          {!modal.editing && (
            <div style={{ marginTop: "1.25rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
              <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", fontSize: "10px", color: C.textSec, letterSpacing: "0.15em" }}>
                <input type="checkbox" checked={isResidencia} onChange={e => setIsResidencia(e.target.checked)} style={{ accentColor: "#C9A96E" }} />
                Residência (múltiplas datas)
              </label>
            </div>
          )}
          ...
          <div style={{ marginTop: "1.75rem", borderTop: `1px solid ${C.borderDim}`, paddingTop: "1.5rem" }}>
            ...
            {artists.map((a, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 130px 90px 32px", gap: "4px", marginBottom: "4px", alignItems: "center" }}>
                <input value={a.nome} onChange={e => updateArtist(i, "nome", e.target.value)} placeholder="Nome..." style={{ ...inputStyle, padding: "0.5rem 0.75rem", fontSize: "11px" }} />
                <CustomSelect value={a.tipo} onChange={v => updateArtist(i, "tipo", v)} options={ARTIST_TIPOS.map(t => ({ value: t, label: t }))} style={{ ...inputStyle, padding: "0.5rem 0.5rem", fontSize: "10px" }} />
                <input value={a.fee} onChange={e => updateArtist(i, "fee", e.target.value)} placeholder="0" type="number" style={{ ...inputStyle, padding: "0.5rem 0.5rem", fontSize: "11px" }} />
                <button type="button" onClick={() => removeArtistRow(i)} style={{ background: "transparent", border: "none", color: "rgba(245,240,232,0.2)", cursor: "pointer", fontSize: "13px", padding: "0.4rem", textAlign: "center" }}>✕</button>
              </div>
            ))}
