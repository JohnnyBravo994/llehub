...
export default function ValoresPage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    border: `1px solid ${C.borderDim}`, color: C.textPrimary, fontFamily: "inherit", fontSize: "12px",
  ...
  const btnStyle: React.CSSProperties = {
    ...
    fontSize: "10px", letterSpacing: "0.18em", padding: "0.52rem 0.65rem", cursor: "pointer",
  ...
  return <>
    ...
    <div className="mob-page-desktop" style={{ minHeight: "100vh", background: C.pageBg, color: C.textPrimary, fontFamily: "'Montserrat','Helvetica Neue',sans-serif", opacity: mounted ? 1 : 0, transition: "opacity 0.6s ease" }}>
      ...
      <main style={{ padding: "2.75rem 3.25rem", maxWidth: "1560px", margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", gap: "1rem" }}>
          <div>
            <p style={{ fontSize: "11px", letterSpacing: "0.4em", color: C.textSec, textTransform: "uppercase", fontWeight: 700, marginBottom: "0.4rem" }}>Master de Valores</p>
            <p style={{ fontSize: "13px", color: C.textMuted, letterSpacing: "0.06em" }}>Apenas serviços e artistas. Materiais e residências são geridos nos respetivos módulos; o contexto é escolhido na Agenda ou nas Leads.</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
            <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Pesquisar serviço..." style={{ ...inputStyle, width: 230, fontSize: "13px" }} />
        ...
        {pendingLoaded && servicosPorCriar.length > 0 && (
          <div style={{ background: C.surface, border: `1px solid ${C.borderDim}`, position: "relative", marginBottom: "1.25rem", padding: "1.2rem" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "flex-start", marginBottom: "0.9rem" }}>
              <div>
                <p style={{ fontSize: "10px", letterSpacing: "0.35em", color: C.goldDim, textTransform: "uppercase", fontWeight: 700, marginBottom: "0.35rem" }}>Serviços por criar na Master</p>
                <p style={{ fontSize: "13px", color: C.textMuted, letterSpacing: "0.04em" }}>Serviços já usados em Agenda/Leads mas ainda sem linha na Master. Criar aqui não altera eventos antigos.</p>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.2fr 90px 110px 1fr 120px", gap: "8px", alignItems: "center" }}>
              {servicosPorCriar.slice(0, 12).map(row => (
                <div key={row.servico} style={{ display: "contents" }}>
                  ...
                  <div style={{ fontSize: "12px", color: C.textMuted }}>{row.total} usos</div>
                  <div style={{ fontSize: "12px", color: C.textSec }}>fee médio {euro(row.fee_medio)}</div>
                  <div style={{ fontSize: "12px", color: C.textMuted }}>{row.primeira_data || "—"} → {row.ultima_data || "—"}</div>
            ...
            {servicosPorCriar.length > 12 && <div style={{ marginTop: "0.75rem", fontSize: "11px", color: C.textMuted }}>A mostrar 12 de {servicosPorCriar.length}. Vai criando para aparecerem os restantes.</div>}
          </div>
        )}
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.borderDim}`, position: "relative", overflowX: "auto" }}>
          ...
          <div style={{ minWidth: 1120 }}>
            <div style={{ display: "grid", gridTemplateColumns: grid, gap: "8px", padding: "0.8rem 1rem", borderBottom: `1px solid ${C.border}` }}>
              {["Serviço", "Formato", "Custo Interno", "Parceiro", "SUD", "Cliente Final", "Notas", "Estado", "Ações"].map(h => <span key={h} style={{ fontSize: "9px", letterSpacing: "0.22em", color: C.goldDim, textTransform: "uppercase", fontWeight: 700 }}>{h}</span>)}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: grid, gap: "8px", alignItems: "center", padding: "1.2rem", background: lightTheme ? "rgba(0,0,0,0.02)" : "rgba(var(--theme-accent-rgb),0.03)" }}>
              ...
              <span style={{ fontSize: "10px", color: C.textMuted, letterSpacing: "0.2em", textTransform: "uppercase" }}>Novo</span>
              <button onClick={createRow} style={{ ...btnStyle, background: C.gold, color: lightTheme ? "#FFFFFF" : "var(--theme-bg)" }}>Criar</button>
            </div>
          </div>
        </div>

        <div style={{ marginTop: "1rem", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.95rem" }}>
    ...
    <div className="mob-shell" style={{ fontFamily: "'Montserrat','Helvetica Neue',sans-serif", color: C.textPrimary, opacity: mounted ? 1 : 0, transition: "opacity 0.6s ease" }}>
      ...
      <div style={{ padding: "1.2rem", borderBottom: `1px solid ${C.borderDim}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.8rem" }}>
          <div>
            <p style={{ fontSize: "11px", letterSpacing: "0.35em", color: C.textSec, textTransform: "uppercase", fontWeight: 700 }}>Master de Valores</p>
            <p style={{ fontSize: "13px", color: C.textMuted, marginTop: "0.4rem", lineHeight: 1.45 }}>Apenas serviços e artistas: custo interno, parceiro, SUD e cliente final. Materiais e residências ficam nos respetivos módulos.</p>
          </div>
          <button
            type="button"
        ...
        <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Pesquisar serviço..." style={{ ...inputStyle, marginTop: "0.8rem", fontSize: "14px" }} />
        ...
        {mobileNewOpen && (
          <div style={{ marginTop: "0.9rem", padding: "0.9rem", background: C.surface, border: `1px solid ${C.border}` }}>
            <div style={{ fontSize: "11px", letterSpacing: "0.22em", color: C.gold, textTransform: "uppercase", fontWeight: 700, marginBottom: "0.75rem" }}>Novo valor</div>
      ...
      {pendingLoaded && servicosPorCriar.length > 0 && (
        <div style={{ padding: "0.9rem 1rem", borderBottom: `1px solid ${C.borderDim}` }}>
          <div style={{ fontSize: "11px", letterSpacing: "0.25em", color: C.gold, textTransform: "uppercase", fontWeight: 700, marginBottom: "0.5rem" }}>Serviços por criar</div>
          {servicosPorCriar.slice(0, 4).map(row => (
            <div key={row.servico} style={{ padding: "0.55rem 0", borderTop: `1px solid ${C.borderDim}` }}>
              <div style={{ fontSize: "14px", fontWeight: 700, color: C.textPrimary }}>{row.servico}</div>
              <div style={{ fontSize: "12px", color: C.textSec, marginTop: "2px" }}>{row.total} usos · fee médio {euro(row.fee_medio)}</div>
              <button onClick={() => criarServicoPendente(row)} style={{ marginTop: "0.45rem", background: lightTheme ? "rgba(0,0,0,0.04)" : "rgba(var(--theme-accent-rgb),0.12)", border: `1px solid ${C.border}`, color: C.gold, fontSize: "11px", padding: "0.45rem 0.65rem", cursor: "pointer" }}>Criar na Master</button>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.75rem 1rem", borderBottom: `1px solid ${C.borderDim}`, background: lightTheme ? "rgba(0,0,0,0.02)" : "rgba(var(--theme-accent-rgb),0.025)" }}>
        <span style={{ fontSize: "12px", color: C.textMuted }}>{rows.filter(row => row.ativo === 1).length} valores ativos</span>
      ...
      <div className="mob-list mob-values-list">
        {filteredRows.filter(row => mobileShowInactive || row.ativo === 1).map(row => {
          ...
          return (
            <div key={row.id} style={{ padding: "1.2rem", borderBottom: `1px solid ${C.borderDim}`, background: editing ? (lightTheme ? "rgba(139,69,19,0.035)" : "rgba(var(--theme-accent-rgb),0.035)") : "transparent" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.7rem" }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: "15px", fontWeight: 700, color: C.textPrimary, overflowWrap: "anywhere" }}>{row.servico}</div>
                  <div style={{ fontSize: "13px", color: C.textSec, marginTop: "0.35rem", lineHeight: 1.45 }}>{row.duracao_formato || "Sem formato"}</div>
                </div>
                <span style={{ flexShrink: 0, fontSize: "10px", letterSpacing: "0.12em", textTransform: "uppercase", padding: "0.3rem 0.45rem", border: `1px solid ${row.ativo === 1 ? C.border : C.borderDim}`, color: row.ativo === 1 ? C.green : C.textMuted }}>
                  {row.ativo === 1 ? "Ativo" : "Inativo"}
                </span>
              </div>
              <div style={{ fontSize: "13px", color: C.textSec, marginTop: "0.45rem", lineHeight: 1.55 }}>Custo: {euro(row.custo_interno)} · Parceiro: {euro(row.valor_parceiro)} · SUD: {euro(row.valor_sud)} · Cliente final: {euro(row.valor_cliente_final)}</div>
              {row.notas && <div style={{ fontSize: "12px", color: C.textMuted, marginTop: "0.35rem", lineHeight: 1.5 }}>{row.notas}</div>}
...
function MobileValorFields({
  ...
  const labelStyle: React.CSSProperties = { display: "block", fontSize: "10px", letterSpacing: "0.16em", color: C.textMuted, textTransform: "uppercase", fontWeight: 700, marginBottom: "0.28rem" };
  const fieldStyle: React.CSSProperties = { minWidth: 0 };
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.65rem" }}>
      <label style={{ ...fieldStyle, gridColumn: "1 / -1" }}><span style={labelStyle}>Serviço</span><input list="servicos-vendidos-list" value={draft.servico} onChange={event => onChange("servico", event.target.value)} placeholder="Nome do serviço" style={{ ...inputStyle, fontSize: "14px" }} /></label>
      <label style={{ ...fieldStyle, gridColumn: "1 / -1" }}><span style={labelStyle}>Formato</span><input value={draft.duracao_formato} onChange={event => onChange("duracao_formato", event.target.value)} placeholder="até 4h" style={{ ...inputStyle, fontSize: "14px" }} /></label>
      <label style={fieldStyle}><span style={labelStyle}>Custo interno</span><input value={draft.custo_interno} onChange={event => onChange("custo_interno", event.target.value)} inputMode="decimal" placeholder="0" style={{ ...inputStyle, fontSize: "14px" }} /></label>
      <label style={fieldStyle}><span style={labelStyle}>Parceiro</span><input value={draft.valor_parceiro} onChange={event => onChange("valor_parceiro", event.target.value)} inputMode="decimal" placeholder="0" style={{ ...inputStyle, fontSize: "14px" }} /></label>
      <label style={fieldStyle}><span style={labelStyle}>SUD</span><input value={draft.valor_sud} onChange={event => onChange("valor_sud", event.target.value)} inputMode="decimal" placeholder="0" style={{ ...inputStyle, fontSize: "14px" }} /></label>
      <label style={fieldStyle}><span style={labelStyle}>Cliente final</span><input value={draft.valor_cliente_final} onChange={event => onChange("valor_cliente_final", event.target.value)} inputMode="decimal" placeholder="0" style={{ ...inputStyle, fontSize: "14px" }} /></label>
      <label style={{ ...fieldStyle, gridColumn: "1 / -1" }}><span style={labelStyle}>Notas</span><textarea value={draft.notas} onChange={event => onChange("notas", event.target.value)} placeholder="Exceções ou observações" rows={3} style={{ ...inputStyle, fontSize: "14px", resize: "vertical" }} /></label>
    </div>
  );
}
...
function Metric({ label, value, C }: { label: string; value: string; C: typeof C_Dark }) {
  return <div style={{ background: C.surface, border: `1px solid ${C.borderDim}`, padding: "1.2rem" }}><div style={{ fontSize: "9px", letterSpacing: "0.35em", color: C.textMuted, textTransform: "uppercase", marginBottom: "0.5rem" }}>{label}</div><div style={{ fontSize: "18px", fontWeight: 700, color: C.textPrimary }}>{value}</div></div>;
}
...
function Nav({ userName, active, onLogout }: { userName: string; active: string; onLogout: () => void }) {
  ...
  const allLinks = [
    ...
    { href: "/valores", label: "Valores" }, { href: "/packs", label: "Packs" }, { href: "/residencias", label: "Residências" }, { href: "/clientes", label: "Clientes" },
  ];
  ...
  return <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.25rem 2.5rem", borderBottom: "1px solid var(--theme-border)", position: "sticky", top: 0, zIndex: 100, background: "var(--theme-nav-bg)", backdropFilter: "blur(12px)" }}>
    <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}><span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.3rem", letterSpacing: "0.35em", color: "var(--theme-accent)", fontWeight: 300 }}>LLE</span><div style={{ display: "flex", gap: "0.25rem" }}>{links.map(l => <a key={l.href} href={l.href} style={{ fontSize: "11px", letterSpacing: "0.25em", padding: "0.5rem 0.65rem", textTransform: "uppercase", fontWeight: 500, color: active === l.href.slice(1) ? "var(--theme-accent)" : "var(--theme-text-muted)", textDecoration: "none", fontFamily: "'Montserrat','Helvetica Neue',sans-serif" }}>{l.label}</a>)}</div></div>
    <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}><span style={{ fontSize: "11px", letterSpacing: "0.3em", color: "var(--theme-text-faint)", textTransform: "uppercase" }}>{userName}</span><button onClick={onLogout} style={{ background: "transparent", border: "1px solid rgba(var(--theme-accent-rgb),0.12)", color: "var(--theme-text-faint)", fontSize: "10px", letterSpacing: "0.4em", padding: "0.5rem 1rem", cursor: "pointer", textTransform: "uppercase", fontFamily: "inherit", fontWeight: 600 }}>SAIR</button></div>
  </nav>;
}
