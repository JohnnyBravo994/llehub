...
export default function PacksPage() {
  ...
  const label: React.CSSProperties = { display: "block", fontSize: 8, letterSpacing: ".16em", textTransform: "uppercase", color: C.muted, marginBottom: 5, fontWeight: 700 };
  const button: React.CSSProperties = { border: `1px solid ${C.border}`, background: "transparent", color: C.text, minHeight: 36, padding: "0 12px", cursor: "pointer", fontFamily: "inherit", fontSize: 9, letterSpacing: ".08em", textTransform: "uppercase" };
  ...
  return <>
    <div className="desk-shell" style={{ minHeight: "100vh", background: C.bg, color: C.text, opacity: mounted ? 1 : 0 }}>
      ...
      <main style={{ marginLeft: 220, padding: "2rem 2.2rem", maxWidth: 1500 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20, marginBottom: 24 }}>
          <div><div style={{ fontSize: 9, letterSpacing: ".26em", textTransform: "uppercase", color: C.accent }}>Produtos comerciais</div><h1 style={{ margin: "8px 0 6px", fontFamily: "'Cormorant Garamond',serif", fontWeight: 400, fontSize: 38 }}>Packs</h1><p style={{ margin: 0, color: C.muted, fontSize: 11 }}>Preço próprio do pack + composição por skills e materiais. Standalones continuam a vir de Colaboradores.</p></div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 10, color: C.muted }}><input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} /> Mostrar inativos</label>
        </div>
        ...
        {loading ? <div style={{ color: C.muted }}>A carregar…</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", gap: 12 }}>
          {visible.map(p => <article key={p.id} style={{ background: C.surface, border: `1px solid ${C.border}`, padding: 16, opacity: p.ativo ? 1 : .5, minWidth: 0 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><div style={{ minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 750, overflow: "hidden", textOverflow: "ellipsis" }}>{p.nome}</div><div style={{ fontSize: 10, color: C.muted, marginTop: 4 }}>{p.descricao || "Sem descrição"}</div></div><span style={{ fontSize: 8, color: p.ativo ? C.success : C.muted }}>{p.ativo ? "ATIVO" : "INATIVO"}</span></div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8, marginTop: 14 }}>
              ...
              <div style={{ padding: 9, border: `1px solid ${C.border}` }}><div style={{ fontSize: 7, color: C.muted, textTransform: "uppercase", letterSpacing: ".1em" }}>Composição</div><div style={{ marginTop: 4, fontSize: 11, fontWeight: 700 }}>{p.componentes?.length || 0} componente{p.componentes?.length === 1 ? "" : "s"}</div></div>
            </div>
            {p.componentes?.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 12 }}>{p.componentes.slice(0,6).map((c,i) => <span key={i} style={{ fontSize: 8, padding: "4px 6px", border: `1px solid ${C.border}`, color: C.muted }}>{c.quantidade > 1 ? `${c.quantidade}× ` : ""}{c.tipo === "material" ? (c.material_nome || c.referencia) : c.referencia}</span>)}</div>}
    ...
    <div className="mob-shell" style={{ minHeight: "100vh", background: C.bg, color: C.text, paddingBottom: 88, opacity: mounted ? 1 : 0 }}>
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: C.bg, borderBottom: `1px solid ${C.border}`, padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ fontSize: 8, color: C.accent, letterSpacing: ".2em" }}>PACKS</div><div style={{ fontSize: 19, fontFamily: "'Cormorant Garamond',serif" }}>Produtos comerciais</div></div><button onClick={openNew} style={{ ...button, background: C.accent, color: lightTheme ? "#fff" : "var(--theme-bg)" }}>+ Novo</button></div>
      <div style={{ padding: 12 }}><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Pesquisar..." style={input} /></div>
      <div>{visible.map(p => <div key={p.id} onClick={() => openEdit(p)} style={{ padding: "14px 14px", borderBottom: `1px solid ${C.border}`, opacity: p.ativo ? 1 : .5 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}><b>{p.nome}</b><span style={{ color: C.accent, fontWeight: 700 }}>{euro(p.valor_cliente_final)}</span></div><div style={{ marginTop: 5, fontSize: 10, color: C.muted }}>{p.componentes?.length || 0} componentes · Parceiro {euro(p.valor_parceria)} · SUD {euro(p.valor_sud)}</div></div>)}</div>
    ...
    {drawer.open && <div onMouseDown={e => { if (e.target === e.currentTarget) setDrawer({ open: false, id: null }); }} style={{ position: "fixed", inset: 0, zIndex: 5000, background: "rgba(0,0,0,.55)", display: "flex", justifyContent: "flex-end" }}>
      <section style={{ width: "min(720px,100vw)", height: "100%", overflowY: "auto", background: C.elevated, color: C.text, padding: "20px 20px 40px", boxSizing: "border-box" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 20 }}><div><div style={{ fontSize: 8, color: C.accent, letterSpacing: ".2em" }}>{drawer.id ? "EDITAR PACK" : "NOVO PACK"}</div><h2 style={{ margin: "5px 0 0", fontFamily: "'Cormorant Garamond',serif", fontWeight: 400, fontSize: 30 }}>{form.nome || "Pack"}</h2></div><button onClick={() => setDrawer({ open: false, id: null })} style={button}>Fechar</button></div>
        ...
        <div style={{ marginTop: 18, fontSize: 9, color: C.accent, fontWeight: 800, letterSpacing: ".16em", textTransform: "uppercase" }}>Faturação do Pack</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 10, marginTop: 9 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginTop: 22, paddingTop: 16, borderTop: `1px solid ${C.border}` }}><div><div style={{ fontSize: 9, color: C.accent, fontWeight: 800, letterSpacing: ".16em", textTransform: "uppercase" }}>Composição</div><div style={{ fontSize: 10, color: C.muted, marginTop: 4 }}>Skills são executantes; materiais são equipamento incluído.</div></div><div style={{ display: "flex", gap: 7 }}><button onClick={addSkill} style={button}>+ Skill</button><button onClick={addMaterial} style={button}>+ Material</button></div></div>
        <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
          {form.componentes.length === 0 && <div style={{ border: `1px dashed ${C.border}`, padding: 16, color: C.muted, fontSize: 10 }}>Sem componentes definidos. O pack pode ser vendido, mas o custo estimado ficará sem composição.</div>}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, marginTop: 20 }}><label style={{ display: "flex", alignItems: "center", gap: 7, color: C.muted, fontSize: 10 }}><input type="checkbox" checked={form.ativo === 1} onChange={e => setForm(f => ({ ...f, ativo: e.target.checked ? 1 : 0 }))} /> Pack ativo</label><button disabled={saving} onClick={save} style={{ ...button, background: C.accent, color: lightTheme ? "#fff" : "var(--theme-bg)", borderColor: C.accent, minWidth: 130 }}>{saving ? "A guardar…" : "Guardar Pack"}</button></div>
      </section>
    </div>}

    {toast && <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9000, background: C.surface, color: C.accent, border: `1px solid ${C.border}`, padding: "12px 16px", fontSize: 10 }}>{toast}</div>}
  </>;
}

function Price({ label, value, C }: { label: string; value: number; C: any }) { return <div style={{ padding: 9, border: `1px solid ${C.border}` }}><div style={{ fontSize: 7, color: C.muted, textTransform: "uppercase", letterSpacing: ".1em" }}>{label}</div><div style={{ marginTop: 4, fontSize: 11, fontWeight: 700 }}>{euro(value)}</div></div>; }
...
function Nav({ userName, active, onLogout }: { userName: string; active: string; onLogout: () => void }) {
  ...
  return <aside style={{ position:"fixed",left:0,top:0,bottom:0,width:220,background:"var(--theme-surface)",borderRight:"1px solid var(--theme-border)",padding:"22px 14px",boxSizing:"border-box",zIndex:100 }}><div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,letterSpacing:".2em",color:"var(--theme-accent)",marginBottom:20}}>LLE</div><nav style={{display:"grid",gap:3}}>{links.map(l=><a key={l.href} href={l.href} style={{padding:"9px 10px",textDecoration:"none",fontSize:10,letterSpacing:".05em",color:active===l.href.slice(1)?"var(--theme-accent)":"var(--theme-text-muted)",background:active===l.href.slice(1)?"rgba(var(--theme-accent-rgb),.08)":"transparent"}}>{l.label}</a>)}</nav><div style={{position:"absolute",bottom:18,left:14,right:14,fontSize:9,color:"var(--theme-text-faint)"}}><div>{userName}</div><button onClick={onLogout} style={{marginTop:7,border:0,background:"transparent",padding:0,color:"var(--theme-danger)",fontSize:8,cursor:"pointer"}}>SAIR</button></div></aside>;
}
